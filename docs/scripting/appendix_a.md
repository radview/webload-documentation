...
1. Click	![](../images/script_guide_119.png) to start recording.
...
1. Click **Add to Cart** to purchase the product.
...
1. In WebLOAD Recorder, click	![](../images/script_guide_120.jpeg) to stop recording the script.
...
1. Create an infoMessage to notify you whether your credit card has been validated or not according to the result parameter value.
...
1. Add the resultParser function to the script, which checks whether your credit card information is valid. The function retrieves the information from document.wlXmls[0] in the results parameter and uses the built-in WebLOAD XML DOM to parse and return XML data to the parameter.
...
1. Drag the JavaScript object Building Block into the script.

1. Add the `rs` parameter that calls the login function from the included `login\_js.js file.`

    `rs = Login()`

1. Save the script as script 4–Ajax and Web services.
...
1. In the webloadmpstore website, enter WebLOAD in the Search field and click **SEARCH**. 
...
1. Check the **Show statistics** checkbox. The script in the script appears as follows:
...
1. After a few nodes have been added to the script, click ![](../images/script_guide_120.jpeg) in WebLOAD Recorder to stop recording the script.
...
1. Drag the JavaScript object Building Block from the Toolbox to the Script Tree. In the Building Block, add a function that receives the `document.wlSource` object and manipulates it to retrieve the statistics. The script is as follows:
...
1. Create an `InfoMessage` to notify you of the statistic values during runtime. The edited script is as follows:
...
1. In each of the AJAX calls recorded in the script, add a call to the newly created function:
...
1. Save the script as script 5-AJAX and JSON.
